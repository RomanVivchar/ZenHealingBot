...
from aiogram.webhook.aiohttp_server import (  # noqa: E402
    SimpleRequestHandler,
    setup_application,
)
from aiohttp import web  # noqa: E402

from app.config.bot_instance import bot  # noqa: E402
...
from app.db.database import (  # noqa: E402
    ...
    create_db_and_tables_debug_individual,
)
from app.handlers.code_words import code_words_router  # noqa: E402
from app.handlers.common import common_router  # noqa: E402
...
async def on_startup(bot: Bot) -> None:
    ...
    log.info(
        "Выполняется on_startup: Запуск приложения и установка вебхука..."
    )  # Добавлен лог
    if settings.webhook_host:
        ...
        try:
            ...
            log.info(
                "Выполняется on_startup: Инициализация базы данных..."
            )  # Добавлен лог
            # Убедись, что create_db_and_tables корректно вызывает Base.metadata.create_all
            await create_db_and_tables_debug_individual()
            log.info(
                "Выполняется on_startup: Инициализация базы данных завершена."
            )  # Добавлен лог
            # ----------------------------

            # --- Установка вебхука ---
            log.info(
                f"Выполняется on_startup: Попытка установить вебхук на: {webhook_url}"
            )  # Добавлен лог
            ...
            log.info(
                f"Выполняется on_startup: Вебхук успешно установлен на: {webhook_url}"
            )  # Добавлен лог
            # -------------------------
            ...
            # print("e.args:", e.args)
            log.error(
                f"Выполняется on_startup: Не удалось установить вебхук на {webhook_url}: {e}",
                exc_info=True,
        ...
        log.warning(
            "Выполняется on_startup: WEBHOOK_HOST не указан в настройках. Пропуск установки вебхука."
        )
        log.warning(
            "Выполняется on_startup: Для локальной разработки без HTTPS и домена используйте polling."
        )
...
async def on_shutdown(bot: Bot) -> None:
    ...
    try:
        ...
        log.error(
            f"Выполняется on_shutdown: Ошибка при удалении вебхука: {e}", exc_info=True
        )  # Добавлен лог
...
async def main() -> None:
    ...
    try:
        ...
        log.critical(
            f"Критическая ошибка при инициализации Bot/Dispatcher: {e}", exc_info=True
        )  # Добавлен лог с уровнем CRITICAL
    ...
    try:
        ...
        log.critical(
            f"Критическая ошибка при настройке вебхук-сервера Aiohttp: {e}",
            exc_info=True,
        )  # Добавлен лог с уровнем CRITICAL
    ...
    log.info(
        f"Попытка запуска Aiohttp веб-сервера на http://{settings.webapp_host}:{settings.webapp_port}"
    )  # Добавлен лог
    ...
    try:
        ...
        log.info(
            f"Aiohttp веб-сервер успешно запущен на http://{settings.webapp_host}:{settings.webapp_port}"
        )  # Добавлен лог
        ...
        log.info(
            "Веб-сервер работает. Ожидание сигнала остановки (Ctrl+C)..."
        )  # Добавлен лог
        await asyncio.Event().wait()  # Ждем, пока событие не будет установлено (Ctrl+C вызывает KeyboardInterrupt)

    except (KeyboardInterrupt, SystemExit):
        log.warning(
            "Получен сигнал остановки (Ctrl+C / SystemExit). Завершение работы..."
        )  # Добавлен лог
    except (
        Exception
    ) as e:  # Перехватываем другие возможные ошибки во время работы сервера
        log.critical(
            f"Критическая ошибка во время работы веб-сервера: {e}", exc_info=True
        )  # Добавлен лог с уровнем CRITICAL
        ...
        # Проверить, что он существует и не закрыт
        # if async_engine and not async_engine.closed:
...
if __name__ == "__main__":
    ...
    try:
        ...
        # которые могли не быть пойманы внутри main()
        log.critical(
            "Необработанное исключение во время выполнения asyncio.run: %s",
            e,
            exc_info=True,
        )  # Добавлен лог с уровнем CRITICAL
